...
export function UserFormComponent(input: { saveAPI: (data: UserCreationData) => Promise<APIResponse<void>>  })
{
    const state = UseState({
        userId: ""
    });
    const apiState = UseDeferredAPI(
        () => input.saveAPI({ userId: state.userId }),
        () => Use(Router).RouteTo("/users/" + state.userId)
    );

    if(apiState.started)
        return apiState.fallback;

    const isValid = (state.userId.trim().length > 0);
    return <>
        <FormField title="User Id">
            <LineEdit link={state.links.userId} />
        </FormField>
...
{
    async function Save(data: UserCreationData)
    {
        const response = await Use(APIService).users.post(data);
        return response;
    }
    return <UserFormComponent saveAPI={Save} />;
}